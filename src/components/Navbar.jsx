...
const MessagesDropdown = ({
  ...
  useEffect(() => {
    chatApi.getConversations().then(r => setConvs(r?.data || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return <div className={`absolute right-0 top-full mt-2 w-[360px] bg-[#090e17]/95 backdrop-blur-xl rounded-xl shadow-[0_0_30px_${accentShadow}] border ${isManager ? 'border-emerald-500/20' : 'border-cyan-500/20'} z-50 overflow-hidden animate-[dropIn_.18s_ease-out]`} style={{
    transformOrigin: 'top right'
          ...
          navigate('/messages');
          onClose();
        }} className={`w-8 h-8 rounded shrink-0 bg-[#02040a] border ${isManager ? 'border-emerald-500/30 hover:border-emerald-400 hover:bg-emerald-900/30 text-emerald-400' : 'border-cyan-500/30 hover:border-cyan-400 hover:bg-cyan-900/30 text-cyan-400'} flex items-center justify-center transition-colors group`} title="OPEN_TERMINAL">
                ...
                <div className="relative mb-3 group">
                    ...
                    <input placeholder="> QUERY_COMM_LINKS..." className={`w-full pl-9 pr-4 py-2 bg-[#02040a] border border-slate-700 ${isManager ? 'focus:border-emerald-500/50 focus:ring-emerald-500/20' : 'focus:border-cyan-500/50 focus:ring-cyan-500/20'} rounded text-xs font-mono text-slate-300 focus:outline-none focus:ring-1 placeholder-slate-600 uppercase tracking-wide transition-all`} />
                </div>
          ...
          id: 'unread',
          label: 'UNREAD_PKTS'
        }].map(t => <button key={t.id} onClick={() => setTab(t.id)} className={`px-3 py-1.5 rounded text-[10px] font-black font-mono tracking-widest uppercase transition-colors border ${tab === t.id ? isManager ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'bg-cyan-900/30 text-cyan-400 border-cyan-500/50 shadow-[0_0_10px_rgba(6,182,212,0.2)]' : 'text-slate-500 border-transparent hover:border-slate-700 hover:text-slate-300'}`}>
                            {t.label}
                        </button>)}
                </div>
            </div>
            ...
            <div className="max-h-[340px] overflow-y-auto custom-scrollbar">
                {loading ? <div className="space-y-1 px-2 py-2">
                        {[1, 2, 3].map(i => <div key={i} className="flex items-center gap-3 px-3 py-2.5 animate-pulse">
                    ...
                    </div> : filtered.length === 0 ? <div className="py-10 text-center flex flex-col items-center">
          ...
          return <button key={conv.id} onClick={() => {
            navigate('/messages');
            onClose();
          }} className={`w-full flex items-center gap-3 px-3 py-2.5 ${isManager ? 'hover:bg-emerald-900/10' : 'hover:bg-cyan-900/10'} transition-colors rounded border-l-2 mb-1 group text-left ${isUnread ? isManager ? 'border-emerald-400 bg-emerald-950/20' : 'border-cyan-400 bg-cyan-950/20' : 'border-transparent hover:border-slate-700'}`}>
                                    {/* Avatar */}
                                    <div className="relative flex-shrink-0">
                                        <div className={`w-10 h-10 rounded border flex items-center justify-center text-white font-black text-xs font-mono overflow-hidden ${isUnread ? isManager ? 'bg-emerald-950 border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-cyan-950 border-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.3)]' : 'bg-[#02040a] border-slate-700'}`}>
                                            {conv.other_user_avatar ? <img src={conv.other_user_avatar} alt={name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" /> : initials}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center justify-between gap-1 mt-1">
                                            ...
                                            {isUnread && <span className={`w-1.5 h-1.5 rounded-full ${isManager ? 'bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,1)]' : 'bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,1)]'} flex-shrink-0 animate-pulse`} />}
                                        </div>
                                    </div>
                                </button>;
        })}
...
const NotificationsDropdown = ({
  ...
  const fetchNotifs = useCallback(() => {
    notificationApi.getNotifications().then(r => setNotifs(r?.data || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
  useEffect(() => {
    fetchNotifs();
  }, [fetchNotifs]);
  ...
  const handleClick = async n => {
    ...
    if (path) navigate(path);else if (n.link) navigate(n.link);
    onClose();
  };
  ...
  const NotifItem = ({
    n
  }) => <button onClick={() => handleClick(n)} className={`w-full flex items-start gap-3 px-3 py-2.5 ${isManager ? 'hover:bg-emerald-900/10' : 'hover:bg-indigo-900/10'} transition-colors rounded border-l-2 mb-1 group text-left ${!n.is_read ? isManager ? 'border-emerald-400 bg-emerald-950/20' : 'border-indigo-400 bg-indigo-950/20' : 'border-transparent hover:border-slate-700'}`}>
            {/* Icon / Avatar */}
            <div className="relative flex-shrink-0 mt-0.5">
                <div className={`w-8 h-8 rounded border flex items-center justify-center text-sm overflow-hidden ${!n.is_read ? isManager ? `bg-emerald-950 border-emerald-500 shadow-[0_0_10px_${accentShadow}] text-emerald-300` : `bg-indigo-950 border-indigo-500 shadow-[0_0_10px_${accentShadow}] text-indigo-300` : 'bg-[#02040a] border-slate-700 text-slate-500'}`}>
                    {n.icon || '(!)'}
                </div>
            </div>
            ...
            {/* Unread dot */}
            {!n.is_read && <div className="flex-shrink-0 mt-1">
  ...
  return <div className={`absolute right-0 top-full mt-2 w-[380px] bg-[#090e17]/95 backdrop-blur-xl rounded-xl shadow-[0_0_30px_${containerShadow}] border ${isManager ? 'border-emerald-500/20' : 'border-indigo-500/20'} z-50 overflow-hidden animate-[dropIn_.18s_ease-out]`} style={{
    transformOrigin: 'top right'
  }}>
            ...
            <div className="px-5 pt-5 pb-2">
                <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-3">
                    ...
                    <button onClick={handleMarkAll} className={`text-[9px] font-black font-mono tracking-widest uppercase text-slate-500 ${isManager ? 'hover:text-emerald-400' : 'hover:text-indigo-400'} transition-colors`}>
                        ACKNOWLEDGE_ALL
                    </button>
                </div>
          ...
          id: 'unread',
          label: 'CRITICAL'
        }].map(t => <button key={t.id} onClick={() => setTab(t.id)} className={`px-3 py-1.5 rounded text-[10px] font-black font-mono tracking-widest uppercase transition-colors border ${tab === t.id ? isManager ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'bg-indigo-900/30 text-indigo-400 border-indigo-500/50 shadow-[0_0_10px_rgba(99,102,241,0.2)]' : 'text-slate-500 border-transparent hover:border-slate-700 hover:text-slate-300'}`}>
                            {t.label}
                        </button>)}
                </div>
            </div>
            ...
            <div className="max-h-[360px] overflow-y-auto custom-scrollbar px-2 py-1">
                {loading ? <div className="space-y-1 py-2">
                        {[1, 2, 3].map(i => <div key={i} className="flex items-center gap-3 px-3 py-2.5 animate-pulse">
...
const Navbar = () => {
  ...
  const IconBtn = ({
    ...
    return <button title={title} onClick={() => toggle(id)} className={`relative w-9 h-9 rounded flex items-center justify-center transition-all ${active ? colorClasses[color].active : 'bg-[#02040a] border border-slate-800 hover:border-slate-600 text-slate-400 hover:text-slate-200'}`}>
                {children}
                {badge > 0 && <span className={`absolute -top-1.5 -right-1.5 flex items-center justify-center text-[#02040a] text-[9px] font-black font-mono h-[18px] min-w-[18px] px-1 rounded border-2 border-[#090e17] ${colorClasses[color].badge}`}>
                        {badge > 9 ? '9+' : badge}
                    </span>}
            </button>;
  };
  ...
  return <nav className="sticky top-0 z-50 w-full border-b" style={{
    background: 'rgba(2,6,23,0.95)',
    borderColor: isAdmin ? 'rgba(168,85,247,0.15)' : isManager ? 'rgba(16,185,129,0.15)' : 'rgba(6,182,212,0.15)',
    backdropFilter: 'blur(12px)'
            ...
            <div className="max-w-[1300px] mx-auto px-4 h-[56px] flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                    <NavLink to="/" aria-label="Home" className="relative flex items-center group">
                        ...
                        <img src={FAFLogo} alt="FAF" className="relative h-7 w-auto object-contain transition-all duration-300" style={{
            filter: 'brightness(0) invert(1)'
                    ...
                    <div className="hidden lg:block relative group">
                        ...
                        <input className={`w-[240px] pl-9 pr-3 py-1.5 rounded bg-[#02040a] border border-slate-800 text-[11px] placeholder-slate-600 focus:outline-none ${isManager ? 'focus:border-emerald-500/50 focus:ring-emerald-500/20' : 'focus:border-cyan-500/50 focus:ring-cyan-500/20'} focus:ring-1 transition-all font-mono text-slate-300 uppercase tracking-widest`} placeholder={t('navbar.search_placeholder')} type="search" />
                    </div>
                </div>

                {/* ── CENTER: Nav tabs ── */}
                {user && <div className="flex items-center h-full flex-1 justify-center gap-1 sm:gap-2">
                        {[{
          to: '/',
          label: t('navbar.home'),
          exact: true,
          ...
          return <NavLink key={to} to={to} className={`hidden sm:flex flex-col items-center justify-center px-4 h-full border-b-2 transition-all font-mono ${active ? isAdmin ? 'border-purple-400 text-purple-400 bg-purple-900/10 shadow-[inset_0_-10px_20px_-15px_rgba(168,85,247,0.3)]' : isManager ? 'border-emerald-400 text-emerald-400 bg-emerald-900/10 shadow-[inset_0_-10px_20px_-15px_rgba(16,185,129,0.3)]' : 'border-cyan-400 text-cyan-400 bg-cyan-900/10 shadow-[inset_0_-10px_20px_-15px_rgba(6,182,212,0.3)]' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800/30'}`}>
                                    {icon}
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 relative">
                    {user ? <>
                            {/* Language Switcher */}
                            <button onClick={toggleLang} title={lang === 'vi' ? t("auto.db_f2e15c") : 'Switch to Vietnamese'} className="w-9 h-9 flex items-center justify-center rounded bg-[#02040a] border border-slate-800 hover:border-cyan-500/50 text-slate-400 hover:text-cyan-400 font-mono text-[10px] font-black transition-all">
                                {lang === 'vi' ? 'VI' : 'EN'}
                            </button>

                            {/* Theme Switcher */}
                            <button onClick={toggleTheme} title={theme === 'dark' ? t("auto.db_f50cd8") : t("auto.db_e9af5f")} className="w-9 h-9 flex items-center justify-center rounded bg-[#02040a] border border-slate-800 hover:border-cyan-500/50 text-slate-400 hover:text-cyan-400 transition-all">
                                {theme === 'dark' ? <svg className="w-4 h-4 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg> : <svg className="w-4 h-4 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>}
                            </button>
                            ...
                            <div className="relative">
                                ...
                                {openPanel === 'messages' && <MessagesDropdown onClose={close} navigate={navigate} user={user} />}
                            </div>
                            ...
                            <div className="relative">
                                ...
                                {openPanel === 'notifications' && <NotificationsDropdown onClose={close} navigate={navigate} onMarkAllRead={() => setUnreadNotifications(0)} user={user} />}
                            </div>
                            ...
                            {/* Profile */}
                            <div className="relative">
                                <button onClick={() => toggle('profile')} className={`relative w-9 h-9 flex items-center justify-center transition-all group/navavt`}>
                                     {/* Tech Frame Background */}
                                    <div className={`absolute inset-0 rounded border transition-all duration-300 ${openPanel === 'profile' ? isAdmin ? 'border-purple-400 bg-purple-950/20 shadow-[0_0_15px_rgba(168,85,247,0.5)] scale-110' : isManager ? 'border-emerald-400 bg-emerald-950/20 shadow-[0_0_15px_rgba(16,185,129,0.5)] scale-110' : 'border-cyan-400 bg-cyan-950/20 shadow-[0_0_15px_rgba(6,182,212,0.5)] scale-110' : isAdmin ? 'border-slate-700 bg-[#02040a] group-hover/navavt:border-purple-500/50' : isManager ? 'border-slate-700 bg-[#02040a] group-hover/navavt:border-emerald-500/50' : 'border-slate-700 bg-[#02040a] group-hover/navavt:border-cyan-500/50'}`}></div>

                                    <div className={`relative w-7 h-7 rounded overflow-hidden flex items-center justify-center ${isAdmin ? 'text-purple-400' : isManager ? 'text-emerald-400' : 'text-cyan-400'} font-black font-mono text-[10px] transition-all ${openPanel === 'profile' ? 'scale-110' : ''}`}>
                                        {user?.avatar_url ? <img src={user.avatar_url} alt="av" className="w-full h-full object-cover mix-blend-luminosity hover:mix-blend-normal transition-all duration-500" /> : getInitials(user)}
                                    </div>
                                 ...
                                 {openPanel === 'profile' && <div className={`absolute right-0 top-full mt-2 w-[240px] rounded bg-[#090e17]/95 backdrop-blur-xl shadow-[0_0_30px_${isAdmin ? 'rgba(168,85,247,0.1)' : isManager ? 'rgba(16,185,129,0.1)' : 'rgba(6,182,212,0.1)'}] z-50 overflow-hidden animate-[dropIn_.18s_ease-out] border ${isAdmin ? 'border-purple-500/30' : isManager ? 'border-emerald-500/30' : 'border-cyan-500/30'}`}>
                                        {/* Top neon line */}
                                        ...
                                        {/* Profile header */}
                                         <div className="px-4 pt-5 pb-3 border-b border-slate-800/80 bg-slate-900/30">
                                            ...
                                            <button onClick={() => {
                  close();
                  navigate(isAdmin ? '/admin/dashboard' : user?.role === 'employer' ? '/task-owner/profiles' : isManager ? '/manager/request' : '/dashboard');
                  ...
                  path: '/wallet'
                }].map(item => <button key={item.path} onClick={() => {
                  close();
                  navigate(item.path);
                                        ...
                                        <div className="border-t border-slate-800/80 p-2">
                                            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-2 rounded text-[10px] font-black font-mono tracking-widest uppercase text-rose-500/80 hover:text-rose-400 hover:bg-rose-950/30 transition-colors border border-transparent hover:border-rose-500/30">
