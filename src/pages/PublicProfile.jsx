...
const CyberCard = ({
  ...
  return <div className={`relative rounded-xl border overflow-hidden ${colors[accent]} ${className}`} style={{
...
const PublicProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setError(t('public_profile.error_load'));
      } finally {
        setLoading(false);
      }
    };
    const fetchFollow = async () => {
      if (currentUser && currentUser.id !== Number(id)) {
        try {
          const res = await userApi.checkFollowStatus(id);
    ...
    if (activeTab === 'posts' && userPosts.length === 0) {
      (async () => {
        try {
          ...
          const r = await postsApi.getPostsByUser(id);
          setUserPosts(r.data || []);
        } catch {} finally {
  ...
  const handleMessageClick = async () => {
    if (!currentUser) {
      toast.error(t('public_profile.error_login_msg'));
      return;
    }
    try {
      await openChat(id);
    } catch {
      toast.error(t('public_profile.error_chat'));
    }
  };
  const handleFollowToggle = async () => {
    try {
      ...
      toast.error(t('public_profile.error_action'));
    }
  };

  /* ── Loading / Error ── */
  if (loading) return <div className="min-h-screen flex items-center justify-center gap-3">
            <div className="w-5 h-5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" />
            <span className="text-slate-400 font-mono text-sm">{t('public_profile.loading_profile')}</span>
        </div>;
  if (error || !profile) return <div className="min-h-screen flex items-center justify-center text-red-400 font-mono">
            {error || t('public_profile.not_found')}
        </div>;
  const isOwnProfile = currentUser?.id === Number(id);
  const statCards = [{
    label: t('public_profile.rating_score'),
    value: Number(profile.rating_avg || 0).toFixed(1),
    suffix: '/ 5',
    accent: 'cyan'
  }, {
    label: t('public_profile.jobs_done'),
    value: profile.total_jobs_done || 0,
    suffix: '',
    accent: 'purple'
  }, {
    label: t('public_profile.hourly_rate'),
    value: `${Number(profile.hourly_rate || 0).toLocaleString()} CRED`,
    suffix: t('public_profile.per_hour'),
    accent: 'amber'
  }];
  ...
  return <div className="min-h-screen" style={{
    marginTop: '-1px'
            ...
            <div className="relative z-10">
                ...
                {/* System bar */}
                <div className="border-b border-cyan-500/15" style={{
        background: 'rgba(2,6,23,0.9)',
        backdropFilter: 'blur(8px)'
      }}>
                    <div className="max-w-5xl mx-auto px-4 py-2 flex items-center gap-3">
                        <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-slate-400 hover:text-cyan-400 transition-colors font-mono text-[11px] font-bold tracking-wider">
            ...
            background: 'linear-gradient(135deg,#0e7490 0%,#6d28d9 50%,#1e40af 100%)'
          }}>
                        ...
                        <div className="px-6 pb-6">
                            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                                ...
                                <div className="flex items-end gap-4">
                                    <div className="w-24 h-24 rounded-2xl ring-4 overflow-hidden flex-shrink-0 -mt-12 relative z-10 shadow-2xl shadow-cyan-500/20" style={{
                  background: 'linear-gradient(135deg,#06b6d4,#9333ea)',
                  ringColor: '#0d1224'
                }}>
                                        {profile.avatar_url ? <img src={profile.avatar_url} alt="av" className="w-full h-full object-cover" /> : <span className="flex items-center justify-center w-full h-full text-3xl font-black text-white">{getInitials(profile.full_name)}</span>}
                                    </div>
                                    <div className="pb-1">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <h1 className="text-2xl font-black text-white tracking-wide uppercase leading-tight">{profile.full_name}</h1>
                                            {profile.tier && <span className={`text-[9px] font-black tracking-widest px-2 py-0.5 rounded border uppercase font-mono ${profile.tier === 'EXPERT' ? 'bg-amber-500/20 text-amber-300 border-amber-500/40' : profile.tier === 'PRO' ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40' : 'bg-slate-700/50 text-slate-400 border-slate-600/40'}`}>{profile.tier}</span>}
                                        </div>
                                ...
                                <div className="flex gap-2 pb-1">
                                    <button onClick={handleMessageClick} className="px-4 py-2 rounded-xl font-black text-[11px] tracking-widest uppercase font-mono transition-all bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white shadow-lg shadow-cyan-500/25 hover:-translate-y-0.5">
                                        {t('public_profile.btn_message')}
                                    </button>
                                    {currentUser && !isOwnProfile && <button onClick={handleFollowToggle} className={`px-4 py-2 rounded-xl font-black text-[11px] tracking-widest uppercase font-mono transition-all border ${isFollowing ? 'border-slate-600/50 text-slate-400 hover:border-red-500/50 hover:text-red-400 bg-slate-800/40' : 'border-cyan-500/40 text-cyan-300 hover:bg-cyan-500/10 hover:border-cyan-400'}`}>
                                            {isFollowing ? t('public_profile.btn_following') : t('public_profile.btn_follow')}
                                        </button>}
                                    <button onClick={() => window.print()} className="px-4 py-2 rounded-xl border border-slate-600/50 text-slate-400 hover:bg-slate-800/40 hover:text-white font-black text-[11px] tracking-widest uppercase font-mono transition-all flex items-center gap-2">
                    ...
                    {/* ── TABS ── */}
                    <div className="flex gap-1 p-1 rounded-xl border border-slate-700/40" style={{
          background: 'rgba(15,23,42,0.8)'
        }}>
                        {[{
            key: 'overview',
            label: t('public_profile.tab_overview', t("auto.db_09c0fd"))
          }, {
            key: 'posts',
            label: t('public_profile.tab_posts', t("auto.db_998bbe"))
          }, {
            key: 'reviews',
            label: t("auto.db_b4292d")
          }].map(tab => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex-1 py-2 px-4 rounded-lg font-mono font-black text-[10px] tracking-widest uppercase transition-all ${activeTab === tab.key ? 'bg-gradient-to-r from-cyan-600/30 to-blue-600/30 text-cyan-300 border border-cyan-500/30 shadow-[0_0_10px_rgba(6,182,212,0.1)]' : 'text-slate-500 hover:text-slate-300 border border-transparent hover:border-slate-600/40'}`}>
                                {tab.label}
                            </button>)}
                    </div>
                    ...
                    {activeTab === 'overview' && <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {statCards.map(s => <CyberCard key={s.label} accent={s.accent} className="p-5 text-center">
                                        <SectionLabel>{s.label}</SectionLabel>
                                        <div className={`text-3xl font-black font-mono ${s.accent === 'cyan' ? 'text-cyan-300' : s.accent === 'purple' ? 'text-purple-300' : 'text-amber-300'}`}>{s.value}</div>
                            ...
                            {/* Skills / Mastery */}
                            {allSkills.length > 0 && <CyberCard accent="purple" className="p-6">
                ...
                return <div key={idx} className="group">
                                                    <div className="flex justify-between items-center mb-2">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[10px] font-black px-2.5 py-0.5 rounded-full font-mono tracking-widest uppercase" style={{
                        color: tier.color,
                        background: tier.bg,
                      ...
                      background: `linear-gradient(90deg, ${tier.color}80, ${tier.color})`
                                                    ...
                                                    {next && <div className="flex justify-between mt-1">
                                                            <span className="text-[9px] font-mono text-slate-600">{tier.name}</span>
                                                            <span className="text-[9px] font-mono" style={{
                      color: next.color
                    }}>→ {next.name} ({next.min} pts)</span>
                                                        </div>}
                                                    {!next && <div className="text-[9px] font-mono text-center mt-1" style={{
                    color: tier.color
                  }}>★ MAX RANK ★</div>}
                                                </div>;
              })}
                            ...
                            {profile.education?.length > 0 && <CyberCard accent="cyan" className="p-6">
                                    ...
                                    <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-cyan-500 before:via-cyan-500/50 before:to-transparent">
                                        {profile.education.map((edu, idx) => <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                            ...
                            {/* Experience Timeline */}
                            {profile.experience?.length > 0 && <CyberCard accent="emerald" className="p-6">
                                    <SectionLabel>
                                        <div className="flex items-center gap-2 text-emerald-400">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>{t("auto.db_5b84f0")}</div>
                                    </SectionLabel>
                                    <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-emerald-500/50 before:to-transparent">
                                        {profile.experience.map((exp, idx) => <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                                ...
                                                <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] p-4 rounded border border-slate-700/50 bg-[#090e17]/80 hover:bg-slate-800/80 hover:border-emerald-500/50 transition-colors shadow-sm ml-4 md:ml-0">
                                                    ...
                                                    {exp.description && <p className="text-[11px] font-mono text-slate-400 leading-relaxed whitespace-pre-wrap">{exp.description}</p>}
                                                </div>
                                            </div>)}
                                    </div>
                                </CyberCard>}
                            ...
                            {/* Portfolio */}
                            {profile.portfolio_items?.length > 0 && <CyberCard accent="amber" className="p-6">
                                    <SectionLabel>{t('public_profile.section_portfolio')}</SectionLabel>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {profile.portfolio_items.map((item, idx) => <div key={idx} className="relative rounded-xl border border-slate-700/50 overflow-hidden aspect-video group">
                                                {item.image_url ? <img src={item.image_url} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center text-slate-600 font-mono text-sm bg-slate-800/50">{t('public_profile.no_image')}</div>}
                    ...
                    {activeTab === 'posts' && <div className="space-y-4">
                            {loadingPosts ? <CyberCard className="p-12 text-center">
                                    <div className="w-8 h-8 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
                                    <p className="text-slate-500 font-mono text-sm">{t('public_profile.loading_data')}</p>
                                </CyberCard> : userPosts.length > 0 ? userPosts.map(post => <div key={post.id} className="relative group">
                                    ...
                                    </div>) : <CyberCard className="p-14 text-center">
                    ...
                    {activeTab === 'reviews' && <div className="space-y-4">
                            {loadingReviews ? <CyberCard className="p-12 text-center">
                                    <div className="w-8 h-8 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
                                    <p className="text-slate-500 font-mono text-sm">{t("auto.db_48a38f")}</p>
                                </CyberCard> : userReviews.length > 0 ? userReviews.map(review => <CyberCard key={review.id} accent="purple" className="p-6">
                                        ...
                                        {review.skillRatings && review.skillRatings.length > 0 && <div className="mt-4 pt-4 border-t border-slate-700/50 flex flex-wrap gap-2">
                                                {review.skillRatings.map((sr, idx) => <span key={idx} className="text-[10px] font-mono px-2 py-1 rounded bg-cyan-900/20 text-cyan-400 border border-cyan-500/30 uppercase tracking-widest flex items-center gap-1">
                                                        {sr.skill_name}: <span className="text-amber-400">{sr.rating}★</span>
                                                    </span>)}
                                            </div>}
                                    </CyberCard>) : <CyberCard className="p-14 text-center">
                                    <div className="text-4xl mb-4">⭐</div>
                                    <h3 className="text-[16px] font-black text-white tracking-widest uppercase font-mono mb-2">{t("auto.db_cb6602")}</h3>
                                    <p className="text-slate-500 text-[12px] font-mono">{t("auto.db_3740cc")}</p>
                                </CyberCard>}
                        </div>}
                </div>
            </div>
