...
const ActiveJob = () => {
  ...
  const fetchActiveContract = async () => {
    try {
      ...
      if (res.data) {
        ...
        const active = contracts.find(c => !['COMPLETED', 'CANCELLED'].includes(c.status) && c.worker_id != null);
        if (active) {
  ...
  const getStatusIcon = status => {
    switch (status) {
      ...
      case 'SUBMITTED':
        return <div className="p-2 bg-yellow-100 rounded-lg">
                        <svg className="w-5 h-5 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                    </div>;
      case 'APPROVED':
        return <div className="p-2 bg-green-100 rounded-lg">
                        <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                    </div>;
      case 'REJECTED':
        return <div className="p-2 bg-rose-500/20 border border-rose-500/50 rounded-lg shadow-[0_0_10px_rgba(244,63,94,0.3)]">
                        <svg className="w-5 h-5 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </div>;
      case 'DISPUTED':
        return <div className="p-2 bg-amber-500/20 border border-amber-500/50 rounded-lg shadow-[0_0_10px_rgba(245,158,11,0.3)] animate-pulse">
  ...
  if (!contract) {
    return <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center max-w-md p-8 bg-white rounded-xl shadow-sm">
                    ...
                    <button onClick={() => navigate('/find-work')} className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">
  ...
  return <div className="min-h-screen bg-gray-50">
            <div className="max-w-6xl mx-auto px-4 py-8">
                {/* Job Finished Banner */}
                {isFinished && <div className={`mb-8 p-8 rounded-[2rem] border flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm hover:shadow-md transition-shadow ${contract.status === 'COMPLETED' ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-200' : 'bg-gradient-to-r from-red-50 to-rose-50 border-red-200'}`}>
                        <div className="flex items-center gap-5">
                            <div className={`p-4 rounded-2xl shadow-sm ${contract.status === 'COMPLETED' ? 'bg-white text-green-600' : 'bg-white text-red-600'}`}>
                                <svg className={`w-8 h-8 ${contract.status === 'COMPLETED' ? 'text-green-600' : 'text-red-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    {contract.status === 'COMPLETED' ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />}
                                </svg>
                            </div>
                            <div>
                                ...
                                <p className={contract.status === 'COMPLETED' ? 'text-green-700' : 'text-red-700'}>
                                    {contract.status === 'COMPLETED' ? 'Great work! This contract has been finalized and fully paid.' : 'This contract was cancelled. No further work can be submitted.'}
                                </p>
                            </div>
                        </div>
                        <button onClick={() => navigate('/dashboard')} className={`px-8 py-3.5 rounded-xl font-bold transition-all shadow-md hover:shadow-lg transform active:scale-95 whitespace-nowrap ${contract.status === 'COMPLETED' ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white hover:from-green-500 hover:to-emerald-500 shadow-green-500/20' : 'bg-gradient-to-r from-red-600 to-rose-600 text-white hover:from-red-500 hover:to-rose-500 shadow-red-500/20'}`}>
                            Return to Dashboard
                        </button>
                    </div>}
                {/* Signature Warning */}
                {!bothSigned && <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-[2rem] p-8 mb-8 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-start gap-5">
                            ...
                            <div className="flex-1">
                                ...
                                {!contract.signature_worker && <button onClick={() => navigate(`/contract/${contract.id}/sign`)} className="mt-6 px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl hover:from-blue-500 hover:to-indigo-500 flex items-center gap-2 shadow-md shadow-blue-500/20 transform active:scale-95 transition-all">
                ...
                {/* Expiration Warning */}
                {isExpired && <div className="bg-gradient-to-r from-red-50 to-rose-50 border border-red-200 rounded-[2rem] p-8 mb-8 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-start gap-5">
                            <div className="flex-shrink-0 p-4 bg-white rounded-2xl shadow-sm">
                    ...
                    <div className="flex items-start justify-between">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                {contract.status === 'ACTIVE' && !isFinished && <button onClick={handleTerminateContract} className="text-xs font-bold text-red-600 hover:text-red-700 hover:underline flex items-center gap-1">
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>{t("auto.db_d299d2")}</button>}
                            </div>
                            <p className="text-gray-600 mb-4">{contract.job_description}</p>
                            <div className="flex items-center gap-6 text-sm mb-4">
                                {contract.job_start_date && <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-6 text-sm">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-green-600">{Number(contract.total_amount || 0).toLocaleString()} CRED</span>
                                </div>
                            </div>
                        </div>
            ...
            return <div key={checkpoint.id} className="rounded-2xl border p-6 relative overflow-hidden flex flex-col transition-all hover:bg-slate-800/20" style={{
              background: 'linear-gradient(145deg,#0d1224,#0f172a)',
              borderColor: 'rgba(51,65,85,0.7)'
            }}>
                                        {/* Status top border */}
                                        <div className={`absolute top-0 inset-x-0 h-1 bg-gradient-to-r ${checkpoint.status === 'APPROVED' ? 'from-emerald-600 via-emerald-400 to-emerald-600' : checkpoint.status === 'REJECTED' ? 'from-rose-600 via-rose-400 to-rose-600' : checkpoint.status === 'DISPUTED' ? 'from-amber-600 via-amber-400 to-amber-600' : checkpoint.status === 'SUBMITTED' ? 'from-amber-600 via-amber-400 to-amber-600' : checkpoint.status === 'IN_PROGRESS' || canSubmit ? 'from-cyan-600 via-cyan-400 to-cyan-600' : 'from-slate-700 via-slate-600 to-slate-700'}`} />
                                        <div className="flex justify-between items-start gap-5">
                                            <div className="flex items-start gap-4">
                                                <div className={`flex items-center justify-center w-10 h-10 rounded shrink-0 border bg-[#02040a] ${checkpoint.status === 'APPROVED' ? 'text-emerald-400 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : checkpoint.status === 'REJECTED' ? 'text-rose-400 border-rose-500/50 shadow-[0_0_15px_rgba(244,63,94,0.2)]' : checkpoint.status === 'DISPUTED' ? 'text-amber-400 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : checkpoint.status === 'SUBMITTED' ? 'text-amber-400 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : checkpoint.status === 'IN_PROGRESS' || canSubmit ? 'text-cyan-400 border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'text-slate-500 border-slate-700'}`}>
                                                    {checkpoint.status === 'APPROVED' ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg> : checkpoint.status === 'REJECTED' ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg> : checkpoint.status === 'DISPUTED' ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg> : checkpoint.status === 'SUBMITTED' ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> : checkpoint.status === 'IN_PROGRESS' || canSubmit ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-3 mb-2">
                                                        <span className="text-[10px] font-black font-mono text-cyan-500 uppercase tracking-widest">CHECKPOINT 0{index + 1}</span>
                                                        <span className={`px-2 py-0.5 rounded border text-[9px] font-black font-mono tracking-widest uppercase ${checkpoint.status === 'APPROVED' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : checkpoint.status === 'REJECTED' ? 'bg-rose-900/30 text-rose-400 border-rose-500/30' : checkpoint.status === 'DISPUTED' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : checkpoint.status === 'SUBMITTED' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : checkpoint.status === 'IN_PROGRESS' || canSubmit ? 'bg-indigo-900/30 text-indigo-400 border-indigo-500/30' : 'bg-slate-800 text-slate-400 border-slate-600'}`}>{checkpoint.status || 'PENDING'}</span>
                                                    </div>
                                            ...
                                            {checkpoint.amount && <div className="text-lg font-black text-cyan-400 font-mono">{Number(checkpoint.amount).toLocaleString()} CRED</div>}
                                        </div>

                                        {/* Submission details */}
                                        {checkpoint.submission_url && <div className="mt-4 pt-4 border-t border-slate-700/50 bg-slate-900/30 rounded-lg p-3">
                                        ...
                                        {checkpoint.review_notes && <div className={`mt-3 border-l-[3px] p-3 rounded-r-lg ${checkpoint.status === 'APPROVED' ? 'border-emerald-500 bg-emerald-900/10' : 'border-rose-500 bg-rose-900/10'}`}>
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-[9px] font-black tracking-widest text-slate-400 uppercase font-mono block">CLIENT REVIEW</span>
                                                    {checkpoint.status === 'REJECTED' && !isFinished && <button onClick={e => {
                    e.stopPropagation();
                    handleOpenDisputeModal(checkpoint);
                                                            ...
                                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>{t("auto.db_ebe40c")}</button>}
                                                </div>
                                                <p className="text-sm text-slate-200 font-mono italic">"{checkpoint.review_notes}"</p>
                                            </div>}

                                        {/* Disputed Status Banner */}
                                        {checkpoint.status === 'DISPUTED' && <div className="mt-3 bg-amber-900/10 border border-amber-500/30 rounded-lg p-4 flex items-start gap-3">
                                        ...
                                        {checkpoint.status === 'SUBMITTED' && <div className="mt-4 pt-4 border-t border-slate-700/50 flex flex-col items-center gap-3">
                                                ...
                                                {!isFinished && <button onClick={() => navigate(`/workspace/checkpoint/${checkpoint.id}`)} className="w-full py-3 border border-amber-500/50 text-amber-400 bg-amber-900/20 hover:bg-amber-900/40 hover:text-amber-300 font-black rounded-xl text-[11px] font-mono transition-all">{t("auto.db_d8eb74")}</button>}
                                            </div>}

                                        {checkpoint.status === 'REJECTED' && !isFinished && <div className="mt-4 pt-4 border-t border-slate-700/50 flex flex-col items-center gap-2">
                                                <p className="text-[10px] text-rose-500 font-mono tracking-widest uppercase mb-2 text-center">{t("auto.db_fa2d2e")}</p>
                                                <button onClick={() => navigate(`/workspace/checkpoint/${checkpoint.id}`)} className="w-full py-3.5 bg-gradient-to-r from-rose-600 to-rose-700 text-white font-black rounded-xl hover:from-rose-500 hover:to-rose-600 transition-all flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(244,63,94,0.3)] transform active:scale-[0.98] text-[11px] font-mono tracking-widest uppercase border border-rose-400/50">{t("auto.db_d664d2")}</button>
                                            </div>}
                                    </div>;
            ...
            {/* Dispute Modal */}
            {showDisputeModal && <div className="fixed inset-0 bg-[#02040a]/90 backdrop-blur-md flex items-center justify-center z-[100] p-4 font-mono">
                    <div className="bg-[#090e17] border border-rose-500/30 rounded-2xl max-w-lg w-full p-8 shadow-[0_0_50px_rgba(244,63,94,0.15)] relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-white uppercase tracking-widest">{t("auto.db_ca7553")}</h3>
                        ...
                        <div className="mb-8 p-4 bg-rose-900/10 border border-rose-500/20 rounded-xl">
                            <p className="text-[11px] text-rose-400 leading-relaxed">{t("auto.db_23145f")}<br />{t("auto.db_6c0d82")}</p>
                        </div>
                        <div className="mb-8">
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">{t("auto.db_9220ea")}<span className="text-rose-500">*</span></label>
                            <textarea value={disputeReason} onChange={e => setDisputeReason(e.target.value)} placeholder={t("auto.db_96b54e")} rows={5} className="w-full px-4 py-3 bg-[#02040a] border border-rose-500/30 rounded-xl focus:outline-none focus:border-rose-400 text-white font-mono text-sm" />
                        </div>
                        <div className="flex gap-4">
                            <button onClick={() => setShowDisputeModal(false)} className="flex-1 px-6 py-4 border border-slate-700 text-slate-500 font-black text-[12px] uppercase rounded-xl hover:bg-slate-800 transition-all">{t("auto.db_20fefd")}</button>
                            <button onClick={handleSubmitDispute} disabled={submitting} className="flex-[2] px-6 py-4 bg-gradient-to-r from-rose-600 to-rose-700 text-white font-black text-[12px] uppercase rounded-xl hover:from-rose-500 hover:to-rose-600 disabled:opacity-50 transition-all shadow-[0_0_20px_rgba(244,63,94,0.3)] border border-rose-400/50">
                                {submitting ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : t("auto.db_8652f5")}
                            </button>
                        </div>
                    </div>
                </div>}
