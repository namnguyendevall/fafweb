...
const AILaserRecommendJob = ({
  ...
  const handleScan = async e => {
    ...
    return (
        <div className="p-3 rounded-xl border border-slate-700/50 hover:border-cyan-500/30 bg-slate-800/30 transition-all cursor-pointer relative overflow-hidden group/ai" onClick={() => navigate(`/work/${job.id}`)}>
            ...
            <div className="relative z-20 mt-2 border-t border-slate-700/50 pt-2">
                {!scanned && !scanning ? (
                    <button onClick={handleScan} className="w-full flex items-center justify-center gap-2 py-1.5 text-[9px] font-black font-mono tracking-widest uppercase text-purple-400 bg-purple-900/20 hover:bg-purple-900/40 border border-purple-500/30 hover:border-purple-400 rounded transition-all shadow-[0_0_10px_rgba(168,85,247,0.1)] group-hover/ai:shadow-[0_0_15px_rgba(168,85,247,0.3)]">
                    ...
                    <div className="w-full text-center py-1.5 text-[9px] font-black font-mono tracking-widest uppercase text-cyan-400 animate-pulse">
                        ...
                        <span className="inline-block animate-[bounce_1s_infinite_200ms]">.</span> {t("dashboard.ai_processing")}
                    </div>
                ) : (
...
const WorkerDashboard = () => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        ...
        const mappedJobs = (res?.data ?? []).map(job => ({
          ...
          price: `${Number(job.budget).toLocaleString()} CRED`,
          meta: [{
            icon: 'calendar',
            text: new Date(job.created_at).toLocaleDateString()
          }, {
            icon: 'spark',
            text: `${job.job_type === 'SHORT_TERM' ? t('dashboard.short_term') : t('dashboard.long_term')}`
          }],
    ...
    const fetchContracts = async () => {
      try {
        ...
        const [activeRes, allRes] = await Promise.all([contractsApi.getMyActiveContract(), contractsApi.getMyContracts()]);
  ...
  return <div className="w-full min-h-screen bg-transparent text-slate-300 relative">

            <div className="mx-auto max-w-7xl px-4 py-8 relative z-10">
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                    {/* ── LEFT SIDEBAR ── */}
                    <aside className="lg:col-span-3">
                        <div className="rounded-2xl border p-5 relative overflow-hidden group" style={{
            ...
            borderColor: 'rgba(6,182,212,0.2)'
                            ...
                            <div className="mb-6 min-w-0">
                                ...
                                {userObj.tier && <span className={`inline-block text-[9px] font-black px-2 py-0.5 rounded font-mono uppercase tracking-widest ${userObj.tier === 'EXPERT' ? 'bg-purple-900/30 text-purple-400 border border-purple-500/30' : userObj.tier === 'PRO' ? 'bg-cyan-900/30 text-cyan-400 border border-cyan-500/30' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>
                ...
                borderColor: 'rgba(6,182,212,0.15)'
                    ...
                    <section className="lg:col-span-9 space-y-6">
                        {/* Header Panel */}
                        <div className="rounded-2xl border p-6 lg:p-8 relative overflow-hidden" style={{
            background: 'radial-gradient(ellipse at bottom right, rgba(6,182,212,0.1) 0%, transparent 60%), linear-gradient(145deg,#0d1224,#0f172a)',
            borderColor: 'rgba(6,182,212,0.2)'
          }}>
                ...
                id: 'applications',
                label: t("dashboard.my_applications")
              }, {
                id: 'reviews',
                label: t('dashboard.performance_reviews')
              }].map(tabDef => <button key={tabDef.id} onClick={() => setActiveTab(tabDef.id)} className={`whitespace-nowrap px-6 py-3 border-b-2 font-black text-[11px] tracking-widest uppercase font-mono transition-all ${activeTab === tabDef.id ? 'border-cyan-400 text-cyan-400 bg-cyan-500/5' : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}>
                                        {tabDef.label}
                                    </button>)}
                            </nav>
                        </div>
                        ...
                        {/* Tab Content */}
                        <div className="pt-2">
                            {activeTab === 'overview' && <div className="space-y-6 animate-[fadeIn_.3s_ease-out]">
                                    {/* Active Mission */}
                                    {activeContract && <div className="rounded-2xl border p-6 mb-6 relative overflow-hidden" style={{
                ...
                borderColor: 'rgba(34,211,238,0.4)'
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-4">
                                                    ...
                                                    <div className="flex flex-col md:items-end gap-2">
                                                        <span className={`inline-block px-3 py-1 rounded text-[10px] font-black font-mono tracking-widest uppercase border ring-1 blur-[0.3px] ${activeContract.status === 'PENDING' || !activeContract.signature_worker ? 'bg-amber-900/40 text-amber-400 border-amber-500/50 ring-amber-400/20' : 'bg-cyan-900/40 text-cyan-400 border-cyan-500/50 ring-cyan-400/20'}`}>
                                                            {activeContract.status === 'PENDING' || !activeContract.signature_worker ? t("dashboard.waiting_signature") : t('dashboard.in_progress')}
                                                        </span>
                                                        <p className="text-lg font-black text-emerald-400 font-mono">{Number(activeContract.total_amount || 0).toLocaleString()} CRED</p>
                                                    </div>
                                                </div>
                                                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                                                    {!activeContract.signature_worker ? <button onClick={() => navigate(`/contract/${activeContract.id}/sign`)} className="w-full sm:w-auto px-6 py-2.5 text-[10px] font-black font-mono tracking-widest uppercase bg-amber-500 hover:bg-amber-400 text-slate-900 transition-colors rounded shadow-[0_0_15px_rgba(245,158,11,0.4)]">{t("dashboard.sign_contract")}</button> : <button onClick={() => navigate(`/contract/${activeContract.id}/view`)} className="w-full sm:w-auto px-6 py-2.5 text-[10px] font-black font-mono tracking-widest uppercase bg-cyan-500 hover:bg-cyan-400 text-[#020617] transition-colors rounded shadow-[0_0_15px_rgba(6,182,212,0.4)]">
                                    ...
                                    {/* Stats Grid */}
                                    {!contractLoading && <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  borderColor: 'rgba(56,189,248,0.2)'
                                                ...
                                                <div className="text-3xl font-black text-white mb-1 font-mono">{(allContracts.filter(c => c.status === 'COMPLETED').reduce((s, c) => s + Number(c.total_amount || 0), 0) * 0.95).toLocaleString()} CRED</div>
                  ...
                  borderColor: 'rgba(52,211,153,0.2)'
                                                ...
                                                <div className="text-[10px] font-mono tracking-widest text-emerald-400 uppercase mb-2">{t('dashboard.ongoing_checkpoints')}</div>
                                                <div className="text-3xl font-black text-white mb-1 font-mono">
                                                    {activeContract ? (activeContract.checkpoints || []).filter(cp => cp.status === 'IN_PROGRESS').length : 0}
                                                </div>
                                                <div className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">{t('dashboard.pending_delivery')}</div>
                                            </div>
                                        </div>}
                                </div>}

                            {activeTab === 'jobs' && <div className="space-y-6 animate-[fadeIn_.3s_ease-out]">
                                    <div className="rounded-2xl border p-6" style={{
                background: 'linear-gradient(145deg,#0d1224,#0f172a)',
                borderColor: 'rgba(6,182,212,0.2)'
              }}>
                                        <SectionLabel>{t('dashboard.all_contracts')}</SectionLabel>
                                        <JobTable contracts={allContracts} type="worker" loading={contractLoading} />
                                    </div>
                                </div>}

                            {activeTab === 'applications' && <div className="space-y-6 animate-[fadeIn_.3s_ease-out]">
                                    <div className="rounded-2xl border p-6" style={{
                background: 'linear-gradient(145deg,#0d1224,#0f172a)',
                borderColor: 'rgba(6,182,212,0.2)'
              }}>
                                        <SectionLabel>{t('dashboard.submitted_proposals')}</SectionLabel>
                                        {proposalsLoading ? <div className="py-10 text-center text-cyan-500 font-mono text-[10px] tracking-widest uppercase animate-pulse">{t("dashboard.loading_proposals")}</div> : myProposals.length > 0 ? <div className="space-y-4">
                                                {myProposals.map(p => <div key={p.id} className="group flex items-center justify-between p-4 rounded-xl border border-slate-800/50 bg-[#02040a]/40 hover:border-cyan-500/30 transition-all cursor-pointer" onClick={() => navigate(`/work/${p.job_id}`)}>
                                                        <div className="min-w-0">
                                                            <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-1 truncate">{p.job_title}</h4>
                                                            <div className="flex items-center gap-4 text-[10px] text-slate-400 font-mono tracking-widest uppercase">
                                                                <span>{t('dashboard.proposed_price')}: <span className="text-cyan-400">{p.proposed_price?.toLocaleString()} CRED</span></span>
                                                                <span>•</span>
                                                                <span>{new Date(p.created_at).toLocaleDateString()}</span>
                                                            </div>
                                                        </div>
                                                        <div className="shrink-0 text-right">
                                                            <span className={`inline-block px-3 py-1 rounded text-[9px] font-black font-mono tracking-widest uppercase border ${p.status === 'PENDING' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : p.status === 'ACCEPTED' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 'bg-red-900/30 text-red-400 border-red-500/30'}`}>{p.status}</span>
                                                        </div>
                                                    </div>)}
                                            </div> : <p className="text-center py-10 text-[11px] text-slate-500 italic font-mono uppercase tracking-widest">{t("dashboard.no_applications")}</p>}
                                    </div>
                                </div>}

                            {activeTab === 'reviews' && <div className="space-y-6 animate-[fadeIn_.3s_ease-out]">
                                    <div className="rounded-2xl border p-6" style={{
                background: 'linear-gradient(145deg,#0d1224,#0f172a)',
                borderColor: 'rgba(6,182,212,0.2)'
              }}>
                                        <SectionLabel>{t('dashboard.your_reviews')}</SectionLabel>
                                        {reviewsLoading ? <div className="py-10 text-center text-cyan-500 font-mono text-[10px] tracking-widest uppercase animate-pulse">{t('dashboard.loading_reviews')}</div> : <ReviewsList data={reviewsData} type="worker" />}
                                    </div>
                                </div>}
                        </div>
                    </section>
