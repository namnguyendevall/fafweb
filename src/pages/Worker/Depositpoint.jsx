  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'zalopay') {
        ...
        error(t("auto.db_b5cfa5"));
      }
    } catch (err) {
      console.error("Payment error:", err);
      error(err.response?.data?.message || t("auto.db_e5f78c"));
    } finally {
      setLoading(false);
    }
  };
  return <div className="w-full min-h-screen bg-transparent text-slate-300 relative flex flex-col font-sans">

            <div className="mx-auto max-w-4xl px-4 py-12 relative z-10">
                {/* Header */}
                <div className="mb-8 pl-2">
                    <button onClick={() => navigate('/wallet')} className="inline-flex items-center gap-2 text-[10px] font-black font-mono text-cyan-500 hover:text-cyan-400 uppercase tracking-widest mb-6 transition-colors group">
                        <svg className="w-4 h-4 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                        </svg>{t("auto.db_8d9094")}</button>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-white uppercase tracking-wider">{t("auto.db_d65f87")}</h1>
                            <p className="text-[11px] font-mono text-cyan-500/70 uppercase tracking-widest mt-1">{t("auto.db_3c9e32")}</p>
                        </div>
                    </div>
                </div>

                {/* Main Content Card */}
                <div className="rounded-2xl border p-8 bg-[#090e17]/80 backdrop-blur-md relative overflow-hidden shadow-2xl" style={{
        borderColor: 'rgba(6,182,212,0.2)'
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-[#02040a] rounded-xl border border-slate-800 p-6 relative">
                            ...
                            <label className="block text-[10px] font-black text-cyan-500 uppercase tracking-widest font-mono mb-4">{t("auto.db_2a70ad")}</label>
                            <div className="flex flex-col md:flex-row items-center gap-4">
                                <div className="flex-1 w-full relative group">
                                    <input type="number" value={points} onChange={handlePointsChange} className="w-full rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 pl-16 text-lg font-black font-mono text-cyan-300 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 transition-all text-right" />
                                ...
                                <div className="flex-1 w-full relative">
                                    ...
                                    <input type="number" value={vndAmount} onChange={handleVndChange} className="w-full rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 pl-[4.5rem] text-lg font-black font-mono text-emerald-300 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 transition-all text-right" />
                                </div>
                            </div>
                            <p className="mt-3 text-[10px] font-mono font-black text-slate-500 tracking-widest uppercase">{t("auto.db_8f84a1")}{exchangeRate.toLocaleString()} VND
                            </p>
                        </div>

                        {/* Payment Method Section */}
                        <div>
                            <label className="block text-[10px] font-black text-cyan-500 uppercase tracking-widest font-mono mb-4">{t("auto.db_854071")}</label>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {/* ZaloPay Option */}
                                <button onClick={() => setPaymentMethod('zalopay')} className={`relative rounded-xl border p-4 text-left transition-all ${paymentMethod === 'zalopay' ? 'border-cyan-500 bg-cyan-900/10 shadow-[0_0_15px_rgba(6,182,212,0.15)]' : 'border-slate-800 bg-[#02040a] hover:border-slate-700'}`}>
                                    {paymentMethod === 'zalopay' && <div className="absolute top-3 right-3">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-[9px] font-mono text-slate-500 mt-1 uppercase tracking-widest">{t("auto.db_ba6742")}</div>
                                        </div>
                                    </div>
                                </button>

                                {/* MoMo Option */}
                                <button onClick={() => setPaymentMethod('momo')} className={`relative rounded-xl border p-4 text-left transition-all ${paymentMethod === 'momo' ? 'border-cyan-500 bg-cyan-900/10 shadow-[0_0_15px_rgba(6,182,212,0.15)]' : 'border-slate-800 bg-[#02040a] hover:border-slate-700'}`}>
                                    {paymentMethod === 'momo' && <div className="absolute top-3 right-3">
                                            <div className="h-4 w-4 rounded-full bg-cyan-500 flex items-center justify-center">
                                                <svg className="w-2.5 h-2.5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                                                </svg>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-[11px] font-black tracking-widest uppercase font-mono text-slate-200">{t("auto.db_7fd8c7")}</div>
                                            <div className="text-[9px] font-mono text-slate-500 mt-1 uppercase tracking-widest">{t("auto.db_05d2f0")}</div>
                                        </div>
                                    </div>
                                </button>
                            </div>
                        ...
                        <div className="bg-[#02040a] rounded-xl border border-slate-800 p-6 space-y-4 font-mono">
                            <div className="flex items-center justify-between text-slate-400 text-[11px] uppercase tracking-widest">
                                <span>{t("auto.db_9a5502")}{points} CRED)</span>
                                <span className="text-white">{subtotal.toLocaleString()} VND</span>
                            </div>
                            <div className="flex items-center justify-between text-slate-400 text-[11px] uppercase tracking-widest">
                                <span>{t("auto.db_62446f")}</span>
                                <span className="text-white">{processingFee.toLocaleString()} VND</span>
                            </div>
                            <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                                <div>
                                    <div className="text-[12px] font-black text-cyan-400 tracking-widest uppercase">{t("auto.db_487a18")}</div>
                                </div>
                        ...
                        <button onClick={handlePayment} disabled={loading} className={`w-full rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 py-4 text-[12px] font-black tracking-widest font-mono text-white shadow-[0_0_20px_rgba(6,182,212,0.3)] border border-cyan-400/50 transition-all hover:scale-[1.02] flex items-center justify-center gap-3 uppercase ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            ...
                            {loading ? t("auto.db_64fc98") : `XÁC NHẬN THANH TOÁN [ ${totalDue.toLocaleString()} VND ]`}
                        </button>
