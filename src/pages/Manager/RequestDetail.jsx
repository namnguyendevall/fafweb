...
const RequestDetail = () => {
  ...
  const handleAccept = async () => {
    try {
      ...
      toast.error(t("auto.db_1a41ed"));
    } finally {
      setProcessing(false);
    }
  };
  const handleReject = async () => {
    const reason = prompt(t("auto.db_332a53"));
    ...
    try {
      ...
      toast.error(t("auto.db_50fe2c"));
    } finally {
      setProcessing(false);
    }
  };
  if (loading) {
    return <div className="p-8 text-center font-mono text-emerald-500 animate-pulse py-40 uppercase tracking-[0.3em]">
                Deep_Scanning_Packet_Header...
            </div>;
  }
  if (!job) {
    return <div className="p-8 text-center font-mono text-rose-500 py-40 uppercase tracking-[0.3em]">
                Error: Packet_Not_Found_In_Registry
            </div>;
  }
  return <div className={`p-8 space-y-8 max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-700 ${processing ? 'opacity-50 pointer-events-none' : ''}`}>
            {/* Header Section */}
            <header className="relative flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 rounded-2xl border border-emerald-500/10 bg-transparent/40 backdrop-blur-md overflow-hidden">
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-3">
                        <button onClick={() => navigate("/manager/request")} className="group flex items-center gap-2 text-emerald-500/50 hover:text-emerald-400 transition-colors text-[10px] font-black font-mono tracking-widest uppercase">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                label: 'TIMESTAMP',
                val: new Date(job.created_at).toLocaleDateString()
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {job.skills?.length > 0 ? job.skills.map(skill => <span key={skill.id} className="px-3 py-1.5 rounded-lg bg-emerald-500/5 border border-emerald-500/20 text-emerald-400 text-[10px] font-black font-mono uppercase tracking-widest ring-1 ring-emerald-500/10">
                                            {skill.name}
                                        </span>) : <span className="text-slate-600 text-[10px] font-mono uppercase tracking-widest">No skill vectors detected</span>}
                                </div>
                            </div>
                            ...
                            {job.checkpoints?.length > 0 && <div className="space-y-3">
                                    <p className="text-[9px] font-mono font-black text-slate-500 uppercase tracking-widest">Checkpoint_Pipeline</p>
                                    <div className="space-y-3">
                                        {job.checkpoints.map((cp, idx) => <div key={cp.id} className="p-4 rounded-lg border border-emerald-500/5 bg-emerald-500/[0.01] space-y-3">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span className="text-[10px] font-black font-mono text-emerald-400">{cp.amount} CRED</span>
                                                </div>
                                                
                                                {/* Submission Data */}
                                                {(cp.submission_url || cp.submission_notes) && <div className="mt-3 pt-3 border-t border-emerald-500/10 space-y-2">
                                                        <p className="text-[8px] font-mono font-black text-slate-600 uppercase tracking-widest">Submission_Evidence</p>
                                                        {cp.submission_url && <a href={cp.submission_url} target="_blank" rel="noreferrer" className="block text-[10px] font-mono text-emerald-500 underline decoration-emerald-500/30 truncate">
                                                                {cp.submission_url}
                                                            </a>}
                                                        {cp.submission_notes && <p className="text-[9px] font-mono text-slate-400 bg-slate-950/50 p-2 rounded border border-emerald-500/5 italic">
                                                                {cp.submission_notes}
                                                            </p>}
                                                    </div>}
                                            </div>)}
                ...
                <div className="space-y-8">
                    <section className="bg-transparent/40 border border-emerald-500/10 rounded-2xl p-8 backdrop-blur-sm sticky top-8">
                        ...
                        {job.status === 'PENDING' && <div className="grid grid-cols-1 gap-4">
                                <button onClick={handleReject} disabled={processing} className="w-full py-4 rounded-xl border border-rose-500/30 text-rose-500 text-[10px] font-black font-mono tracking-[0.3em] uppercase hover:bg-rose-500/10 transition-all active:scale-95 disabled:opacity-50">
                                    {processing ? 'EXECUTING...' : 'ABORT_INJECTION'}
                                </button>
                                <button onClick={handleAccept} disabled={processing} className="w-full py-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-[#020617] text-[10px] font-black font-mono tracking-[0.3em] uppercase shadow-[0_0_30px_rgba(16,185,129,0.2)] transition-all active:scale-95 disabled:opacity-50">
