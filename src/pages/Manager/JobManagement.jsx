...
const JobManagement = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      toast.error(t("auto.db_0af906"));
    } finally {
      setLoading(false);
    }
  };
  const filteredJobs = jobs.filter(j => j.id.toString().includes(query) || j.title?.toLowerCase().includes(query.toLowerCase()) || j.client_email?.toLowerCase().includes(query.toLowerCase()) || j.worker_email?.toLowerCase().includes(query.toLowerCase()));
            ...
            <header className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="w-full lg:w-[480px] relative group">
                    ...
                    <div className="relative flex items-center bg-transparent/50 border border-emerald-500/10 rounded-xl overflow-hidden backdrop-blur-md">
                        ...
                        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="SEARCH_BY_ID_CREATOR_OR_WORKER..." className="w-full bg-transparent px-4 py-4 text-xs font-mono font-black text-emerald-400 placeholder:text-slate-700 outline-none uppercase tracking-widest" />
                    </div>
                </div>
            </header>

            <div className="grid grid-cols-1 gap-4">
                {loading ? <div className="py-20 text-center font-mono text-emerald-500 animate-pulse uppercase tracking-[0.3em]">
                        Scanning_Active_Contracts...
                    </div> : filteredJobs.length > 0 ? <div className="overflow-x-auto rounded-2xl border border-emerald-500/10 bg-transparent/40 backdrop-blur-md">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                {filteredJobs.map(j => {
              const progress = j.total_checkpoints > 0 ? Math.round(j.approved_checkpoints / j.total_checkpoints * 100) : 0;
              return <tr key={j.id} onClick={() => navigate(`/manager/request/${j.id}`)} className="group border-b border-emerald-500/5 hover:bg-emerald-500/[0.03] transition-colors cursor-pointer">
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <span className="text-xs font-mono font-black text-white group-hover:text-emerald-400 transition-colors">#{j.id}</span>
                                                    {j.has_dispute && <span className="animate-pulse flex items-center justify-center w-4 h-4 rounded-full bg-rose-500/20 border border-rose-500/50 text-rose-500 text-[8px] font-black">!</span>}
                                                </div>
                                            </td>
                                            <td className="p-4">
                                            ...
                                            <td className="p-4">
                                                <div className="w-48 space-y-2">
                                                    ...
                                                    <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden border border-emerald-500/10">
                                                        <div className={`h-full transition-all duration-1000 ${j.has_dispute ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]' : 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]'}`} style={{
                        width: `${progress}%`
                      }}></div>
                                                    </div>
                                                </div>
                    ...
                    </div> : <div className="p-20 text-center rounded-3xl border border-dashed border-emerald-500/20 bg-emerald-500/[0.01]">
