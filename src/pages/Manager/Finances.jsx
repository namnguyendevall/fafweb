...
const Finances = () => {
  ...
  const fetchFinancialData = async () => {
    try {
      setLoading(true);
      const [statsRes, finRes, transRes] = await Promise.all([managerApi.getStats(), managerApi.getFinancials(), managerApi.getTransactions(1, 10)]);
      setStats(statsRes.data);
      setFinancials(finRes.data);
      setTransactions(transRes.data);
    } catch (error) {
      console.error("Failed to fetch financial data:", error);
      toast.error(t("auto.db_7662d5"));
    } finally {
      setLoading(false);
    }
  };
        ...
        backgroundSize: '24px 24px'
                ...
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        ...
                        <button onClick={fetchFinancialData} className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-[#020617] text-[10px] font-black font-mono tracking-widest uppercase transition-all shadow-[0_0_25px_rgba(16,185,129,0.3)]">
                            SYNC_LEDGER
                        </button>
                    </div>
                </div>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[{
        label: 'TOTAL_TURNOVER',
        value: `${Number(financials?.total_turnover || 0).toLocaleString()} CRED`,
        trend: '+0.0%',
        sub: 'CUMULATIVE_VOLUME'
      }, {
        label: 'TOTAL_JOBS',
        value: stats?.total_jobs || 0,
        trend: `+${stats?.pending_jobs || 0}`,
        sub: 'IN_PIPELINE'
      }, {
        label: 'PROTOCOL_REVENUE',
        value: `${Number(financials?.total_fees || 0).toLocaleString()} CRED`,
        trend: '+5%',
        sub: 'SYSTEM_FEE_LOG',
        main: true
      }].map((stat, i) => <div key={i} className={`relative p-8 rounded-2xl border transition-all duration-500 overflow-hidden ${stat.main ? 'bg-emerald-500/[0.07] border-emerald-500/30 shadow-[0_0_40px_rgba(16,185,129,0.05)]' : 'bg-transparent/40 border-emerald-500/5 hover:border-emerald-500/20'}`}>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 p-8 rounded-3xl border border-emerald-500/10 bg-transparent/40 backdrop-blur-sm">
                    ...
                    <div className="h-64 relative group">
                        <svg viewBox="0 0 800 200" className="w-full h-full preserve-3d">
                            ...
                            {/* Grid Lines */}
                            {[0, 50, 100, 150, 200].map(y => <line key={y} x1="0" y1={y} x2="800" y2={y} stroke="#10b981" strokeOpacity="0.05" strokeWidth="1" />)}
                            {/* Area */}
                            <path d="M0 150 C 100 120, 200 130, 300 100 C 400 140, 500 80, 600 120 C 700 180, 750 110, 800 160 L 800 200 L 0 200 Z" fill="url(#emGrad)" className="animate-in fade-in duration-1000" />
                            {/* Line */}
                            <path d="M0 150 C 100 120, 200 130, 300 100 C 400 140, 500 80, 600 120 C 700 180, 750 110, 800 160" fill="none" stroke="#10b981" strokeWidth="3" strokeLinecap="round" className="drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                        </svg>
                        {/* Hover Overlay Mock */}
                ...
                <div className="p-8 rounded-3xl border border-emerald-500/10 bg-transparent/40 backdrop-blur-sm flex flex-col">
                    ...
                    <div className="flex-1 space-y-4">
                        {[{
            label: 'WORKERS',
            val: stats?.total_workers || 0,
            color: 'bg-emerald-500'
          }, {
            label: 'EMPLOYERS',
            val: stats?.total_employers || 0,
            color: 'bg-blue-500'
          }, {
            label: 'MANAGERS',
            val: stats?.total_managers || 0,
            color: 'bg-amber-500'
          }].map(role => <div key={role.label} className="space-y-1.5">
                                ...
                                <div className="h-1 w-full bg-slate-800 rounded-full overflow-hidden">
                                    <div className={`h-full ${role.color} transition-all duration-1000`} style={{
                width: `${role.val / (stats?.total_users || 1) * 100}%`
              }}></div>
                                </div>
                            </div>)}
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-emerald-500/5">
                            {transactions.length > 0 ? transactions.map((log, i) => <tr key={i} className="hover:bg-emerald-500/[0.01] transition-colors group">
