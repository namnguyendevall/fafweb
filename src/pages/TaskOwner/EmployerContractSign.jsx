...
const EmployerContractSign = () => {
  ...
  const handleRequestOtp = async () => {
    try {
      ...
      setOtpSent(true);
      setCountdown(60);
      toast.success(t("auto.db_7a8e4b"));
    } catch (error) {
      toast.error(t("auto.db_642748"));
    }
  };
  useEffect(() => {
    fetchContract();
  ...
  const handleSign = async () => {
    if (!agreed) {
      toast.warning(t("auto.db_d0a005"));
      return;
    }
    if (!otp) {
      toast.warning(t("auto.db_67f260"));
      return;
    }
    try {
      setSigning(true);
  ...
  return <div className="min-h-screen bg-transparent">
            ...
            <div className="flex-1 overflow-auto">
                <div className="max-w-4xl mx-auto px-6 py-8">
                    {/* Header */}
                    <div className="mb-8">
                        <button onClick={() => navigate('/task-owner')} className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 font-medium text-sm mb-4 transition-colors">
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex flex-col">
                                    <p className="font-bold text-gray-900 text-2xl">{Number(contract.total_amount || 0).toLocaleString()} CRED</p>
                                    <p className="text-xs text-blue-600 font-bold italic">{t("auto.db_90d3ed")}{Number((contract.total_amount || 0) * 0.95).toLocaleString()}{t("auto.db_37e9cd")}</p>
                                </div>
                            </div>
                            <div className="space-y-1">
                                <p className="text-sm font-medium text-gray-500">Status</p>
                                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold ${contract.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : contract.status === 'DRAFT' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>
                                    <div className={`w-2 h-2 rounded-full ${contract.status === 'ACTIVE' ? 'bg-green-600' : contract.status === 'DRAFT' ? 'bg-yellow-600' : 'bg-gray-600'}`}></div>
                                    {contract.status}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-gray-200 pt-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className={`p-4 rounded-xl border-2 transition-all ${workerSigned ? 'bg-green-50 border-green-300' : 'bg-gray-50 border-gray-200'}`}>
                                    <div className="flex items-center gap-3 mb-2">
                                        {workerSigned ? <div className="p-2 bg-green-100 rounded-lg">
                                                <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div> : <div className="p-2 bg-gray-100 rounded-lg">
                                ...
                                <div className={`p-4 rounded-xl border-2 transition-all ${employerSigned ? 'bg-green-50 border-green-300' : 'bg-yellow-50 border-yellow-300'}`}>
                                    <div className="flex items-center gap-3 mb-2">
                                        {employerSigned ? <div className="p-2 bg-green-100 rounded-lg">
                                                <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                                </svg>
                    ...
                    {/* Checkpoints */}
                    {contract.checkpoints && contract.checkpoints.length > 0 && <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                            ...
                            <div className="space-y-3">
                                {contract.checkpoints.map((checkpoint, idx) => <div key={checkpoint.id} className="flex justify-between items-center p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div className="flex items-start gap-3 flex-1">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                {checkpoint.due_date && <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                                        ...
                                        <div className="text-right flex flex-col items-end">
                                            <p className="font-bold text-gray-900 text-lg">{Number(checkpoint.amount).toLocaleString()} CRED</p>
                            ...
                            <div className="max-w-4xl mx-auto">
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-8 p-6 bg-white rounded-lg border border-gray-200 shadow-sm">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-3xl font-bold text-gray-900">{Number(contract.total_amount || 0).toLocaleString()} CRED</div>
                                        <div className="text-xs text-blue-600 font-bold mt-1 italic">{t("auto.db_cbaa68")}{Number((contract.total_amount || 0) * 0.95).toLocaleString()}{t("auto.db_07a9cd")}</div>
                                    </div>
                                </div>
                                ...
                                <div className="prose prose-sm max-w-none">
                                    <div className="bg-white rounded-lg border border-gray-200 p-8 shadow-sm">
                                        ...
                                        <div className="text-gray-700 leading-relaxed prose prose-sm max-w-none
                                                ...
                                                prose-ol:list-decimal prose-ol:ml-6 prose-ol:text-sm" style={{
                    fontSize: '0.9375rem',
                    lineHeight: '1.75',
                    ...
                    __html: processNetAmounts(contract.contract_content) || 'No contract content available.'
                        ...
                        <div className="bg-gray-100 px-8 py-4 border-t border-gray-200">
                            <div className="flex items-center justify-between text-xs text-gray-600">
                                ...
                                {contract.created_at && <span>Generated: {new Date(contract.created_at).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                })}</span>}
                    ...
                    {!workerSigned && <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-300 rounded-xl p-6 mb-6">
                    ...
                    {employerSigned && <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-6 mb-6">
                            <div className="flex items-start gap-4">
                    ...
                    {/* Sign Section */}
                    {canSign && <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                            <div className="flex items-start gap-3 mb-6">
                                <input type="checkbox" id="agree" checked={agreed} onChange={e => setAgreed(e.target.checked)} className="mt-1 w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer" />
                            ...
                            <button onClick={() => setShowConfirm(true)} disabled={!agreed} className={`w-full py-3 font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${agreed ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm hover:shadow-md' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                    ...
                    {/* Action Buttons */}
                    <div className="mt-6">
                        <button onClick={() => navigate('/task-owner')} className="px-6 py-3 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-colors">
                            Back to Dashboard
                        </button>
                    </div>
                </div>
            ...
            {/* Confirmation Modal */}
            {showConfirm && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl">
                        <div className="flex items-start gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-gray-900 mb-2">{t("auto.db_0ae8b6")}</h3>
                                <p className="text-gray-700 text-sm">{t("auto.db_363a2a")}</p>
                            </div>
                        </div>

                        <div className="mb-6">
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{t("auto.db_2d595e")}</label>
                            <div className="flex gap-2">
                                <input type="text" value={otp} onChange={e => setOtp(e.target.value)} placeholder="••••••" className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-blue-600/10 focus:border-blue-600 text-center text-xl font-mono tracking-[0.5em] transition-all outline-none" maxLength={6} />
                                <button onClick={handleRequestOtp} disabled={countdown > 0} className="px-4 py-2 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all disabled:opacity-50 text-xs min-w-[120px]">
                                    {countdown > 0 ? `${countdown}s` : otpSent ? t("auto.db_11a40e") : t("auto.db_a3e58d")}
                                </button>
                            </div>
                        </div>

                        <div className="flex gap-3">
                            <button onClick={() => setShowConfirm(false)} className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-colors" disabled={signing}>
                                Cancel
                            </button>
                            <button onClick={handleSign} className="flex-1 px-4 py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2" disabled={signing}>
                                {signing ? <>
