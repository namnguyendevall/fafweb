...
export default function JobListPage() {
  ...
  const getStatusColor = status => {
    switch (status) {
      ...
      case 'COMPLETED':
        return 'bg-transparent text-gray-700 border-gray-200';
      case 'CANCELLED':
                ...
                <header className="sticky top-0 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-gray-100 dark:border-slate-800 shadow-sm px-6 py-5">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 max-w-5xl mx-auto w-full">
                        ...
                        <div className="flex items-center gap-3">
                            <button onClick={() => navigate("/task-owner/post-job")} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-sm font-bold shadow-sm shadow-blue-500/20 transition-all flex items-center gap-2">
                ...
                <main className="flex-1 p-6 overflow-y-auto">
                    <div className="max-w-5xl mx-auto">
                        {loading ? <div className="flex justify-center items-center py-20">
                            ...
                            </div> : jobs.length === 0 ? <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-12 text-center shadow-sm">
                                ...
                                <button onClick={() => navigate("/task-owner/post-job")} className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold transition-colors">
                                    Post a Job Now
                                </button>
                            </div> : <div className="grid gap-5">
                                {jobs.map(job => <div key={job.id} className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-3xl p-6 shadow-sm hover:shadow-lg transition-all">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 border-t border-b border-gray-100 dark:border-slate-700 mb-5">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm font-semibold text-gray-900 dark:text-white">{Number(job.budget).toLocaleString()} CRED</p>
                                            </div>
                                            <div>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                            <div className="flex flex-wrap gap-2">
                                                {job.skills && job.skills.length > 0 ? job.skills.map(skill => <span key={skill.id} className="px-2.5 py-1 text-xs font-bold rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800/50">
                                                            {skill.name}
                                                        </span>) : <span className="text-xs italic text-gray-400 dark:text-gray-500">No skills specified</span>}
                                            </div>
                                            <div className="flex gap-2 shrink-0">
                                                <Link to={`/task-owner/job/${job.id}`} className="px-4 py-2 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-200 text-sm font-bold rounded-xl transition-colors">
                                                    View Details
                                                </Link>
                                                {job.status === 'OPEN' && <button className="px-4 py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-sm font-bold rounded-xl transition-colors">
                                                        Edit Job
                                                    </button>}
                                            </div>
                                        </div>
